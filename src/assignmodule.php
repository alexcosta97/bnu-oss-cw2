...
if (isset($_SESSION['id'])) {
   ...
   if (isset($_POST['selmodule'])) {
      ...
      $data['content'] .= "<p>The module " . $_POST['selmodule']
   ...
   else  // If a module has not been selected
   {
     ...
     $data['content'] .= "<form name='frmassignmodule' action=''
     class='mt-1' method='post' >";
     $data['content'] .= "<div class='form-row'>";
     $data['content'] .= "<div class='form-group col-6'";
     $data['content'] .= "<label for='selmodule'>Select a module
     to assign</label>";
     $data['content'] .= "<select name='selmodule'
     id='selmodule' class='form-control'>";
     // Display the module name sin a drop down selection box
     while($row = $sql -> fetch()) {
        $data['content'] .= "<option value='$row[modulecode]
        '>$row[name]</option>";
     }
     $sql -> closeCursor();
     $data['content'] .= "</select>";
     $data['content'].= "</div></div>";
     $data['content'] .= "<input type='submit' class='btn
     btn-primary btn-md' name='confirm' value='Save' />";
     $data['content'] .= "</form>";
   }
