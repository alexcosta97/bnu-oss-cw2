<?php
   ...
   if (isset($_SESSION['id'])) {
      ...
      $sql = $conn -> prepare('Select * from studentmodules sm,
      module m where m.modulecode = sm.modulecode and
      sm.studentid = ?');
      ...
      $data['content'] .=
      "<tr><th>Code</th><th>Type</th><th>Level</th></tr>";
      $data['content'] .= "</thead>";
      $data['content'] .= "<tbody>";
      // Display the modules within the html table
      while($row = $sql -> fetch()) {
         $data['content'] .= "<tr><td> $row[modulecode]
         </td><td> $row[name] </td>";
